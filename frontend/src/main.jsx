...
const ENABLE_IN_DEV = String(import.meta.env.VITE_SENTRY_ENABLE_IN_DEV || '').toLowerCase() === 'true'
...
if (SENTRY_DSN) {
  window.addEventListener('error', (event) => {
    try {
      ...
      Sentry.captureException(event.error || new Error(String(event.message || 'Unknown error')))
    } catch (e) {
      // ignore
    }
  })
